...
/*
    ...
    这是专为新版SoC制作的LCD控制�?
    仅�?�虑地址�?16位（读写32位对齐）�?
        0x0000: 只写，向其中�?0为复位并切换到文本模�?(default)，写1为复位并切换到图形模�?
        0x0004: 文本模式下的颜色寄存器（仅低8位有效，0~3背景�?4~7前景�?
        0x0008: 字符写寄存器，只写，�?8位为字符，接下来8位保留，接下�?8位行地址，接下来8位列地址
        0x000c: 像素随机写寄存器，只写，�?16位为像素�?
        0x0010: 光标寄存器，�?16位行，高16位列
        0x0014: 像素位置寄存器，�?16位y，高16位x
        0x0018: dma address
        0x001c: dma length, 写它会立刻启动dma
        0x0020: dma ack, 写它会ack dma中断
        0x0024: dma region topleft, �?16位y，高16位x
        0x0028: dma region bottomright, �?16位y，高16位x
*/
...
(
    ...
    output[31:0]    dma_length,       //保证�?6位为0�?64字节对齐�?
    output          dma_iack,       //中断响应
    //连到设备控制器的16位数据�?�道
    output          dma_ready_dev,
    input           dma_valid_dev,
    input           dma_last_dev,       //Note：只要其有效，设备应当立刻停止接�?
        ...
        if(~aresetn) begin
            ...
            1: begin
                if(awvalid) begin
                    ...
                    if(cursor_counter == 20000000) begin
                        cursor_counter<=0;
                        setcur_x<=text_x;
                        setcur_y<=text_y;
                        cursor<=~cursor;
            ...
            4: begin
                if(bready) begin
                    if(sim) begin
                        if((cached_addr[15:0] == `LCDADDR_CHAR) & (cached_wdata[23:16] < `LCD_TEXT_HEIGHT) & (cached_wdata[31:24] < `LCD_TEXT_WIDTH)) begin

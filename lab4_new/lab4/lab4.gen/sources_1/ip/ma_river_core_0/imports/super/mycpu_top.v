...
module ma_river_core(
    ...
    wire[63:0]    fpu_rob_wb_result;              //虽然但是，这个涉及到如何commit，还是要写到fpu_rob�?
    wire[$clog2(`ROB_SIZE) - 1:0] fpu_rob_wb_num;
    wire[3:0]     fpu_rob_wb_waddr;               //FPR
    wire[4:0]     fpu_rob_wb_wprf0;
    wire[4:0]     fpu_rob_wb_wprf1;
    wire[`FPU_GLOBAL_CNT - 1:0]   fpu_rob_wb_gctrl;           //注意：fpu_rob在此之前（一般）不知道该fpu指令控制信号，需要把Commit�?�?必要控制信号写回
    wire[$clog2(`CNT_EXCEPTION) - 1:0]    fpu_rob_wb_excode;  //只会报RI和FPE两种异常
    wire[`FPECNT - 1:0]           fpu_rob_wb_fpeword;         //浮点异常状�?�字
    ...
    wire fpu_frommu_notlast;     //Note: 对于ldc1，�?�出第一个word时有效，表示不要置ready
